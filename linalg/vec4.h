...
#define VEC4_GET(_v, _i)                     \
    ((__builtin_constant_p(_i) && (_i) == 0) \
        ? _mm_cvtss_f32((_v).vec)            \
        : _mm_extract_ps((_v).vec, (_i))
#endif 

#define VEC4_SET(_v, _i, _val)               \
    do { (_v).vec = _mm_blend_ps((_v).vec, _mm_set1_ps(_val), 1 << (_i)); } while (0)

C_UTILS_INLINE static inline vec4f vec4_add(vec4f a, vec4f b) {
    return (vec4f) { .vec = _mm_add_ps(a.vec, b.vec) };
}

C_UTILS_INLINE static inline vec4f vec4_adds(vec4f a, f32 scalar) {
    __m128 vec_b = _mm_load1_ps(&scalar);
    return vec4_add(a, *(vec4f *) &vec_b);
}

C_UTILS_INLINE static inline vec4f vec4_sub(vec4f a, vec4f b) {
    return (vec4f) { .vec =  _mm_sub_ps(a.vec, b.vec) };
}

C_UTILS_INLINE static inline vec4f vec4_subs(vec4f a, f32 scalar) {
    __m128 vec_b = _mm_load1_ps(&scalar);
    return vec4_sub(a, *(vec4f *) &vec_b);
}

C_UTILS_INLINE static inline vec4f vec4_mul(vec4f a, vec4f b) {
    return (vec4f) { .vec = _mm_mul_ps(a.vec, b.vec) };
}

C_UTILS_INLINE static inline vec4f vec4_muls(vec4f a, f32 scalar) {
    __m128 vec_b = _mm_load1_ps(&scalar);
    return vec4_mul(a, *(vec4f *) &vec_b);
}

C_UTILS_INLINE static inline vec4f vec4_div(vec4f a, vec4f b) {
    return (vec4f) { .vec =  _mm_div_ps(a.vec, b.vec) };
}


C_UTILS_INLINE static inline vec4f vec4_divs(vec4f a, f32 scalar) {
    return vec4_muls(a, 1.0F / scalar);
}

C_UTILS_INLINE static inline f32 vec4_hsum(vec4f v) {
    return _mm_hsum_ps(v.vec);
}

C_UTILS_INLINE static inline f32 vec4_length(vec4f a) {
    a = vec4_mul(a, a);
    return sqrtf(vec4_hsum(a));
}

C_UTILS_INLINE static inline vec4f vec4_normalize(vec4f a) {
...
C_UTILS_INLINE static inline f32 vec4_dot(vec4f a, vec4f b) {
    b = vec4_mul(a, b);
    return vec4_hsum(b);
}

C_UTILS_INLINE static inline vec4f vec4_inverse(vec4f a) {
...
C_UTILS_INLINE static inline vec4f vec4_min(vec4f a, vec4f b) {
    return (vec4f) { .vec = _mm_min_ps(a.vec, b.vec) };
}

C_UTILS_INLINE static inline vec4f vec4_max(vec4f a, vec4f b) {
    return (vec4f) { .vec = _mm_max_ps(a.vec, b.vec) };
}

C_GUARD_END()

#endif //C_UTILS_VEC4_H
